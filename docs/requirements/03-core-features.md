...
- 将单次扫描结果写入 SQLite 扫描历史库。
- 默认数据库路径：`~/.config/argus/argus.db`。
- 每次扫描记录包含：时间戳、根路径、总大小、路径记录、文件类型和必要的文件系统元数据。
- scan 的唯一用途是显示当前文件系统的空间分布（ncdu 风格），不用于计算 delta。

### 1.3 体积变化追踪（Delta）

**架构决策（2026-07）**：快照对比（Snapshot Diff）已被移除。Delta 不再通过对比两份全量快照计算。

替代方案：由 **argusd 守护进程**（Phase 3+）负责：

- 全量扫描作为基准（首次，或按需）。
- 利用系统级 API（FSEvents / inotify）捕获文件变更事件。
- 以混合粒度记录：
  - **小文件变化**：按目录聚合 (`/a/b, +1024`)
  - **大文件变化**：按文件单独记录 (`/a/b/big.iso, +1048576`)
- 查询时按时间范围聚合路径的 delta：`WHERE path LIKE ? AND timestamp BETWEEN ? AND ? GROUP BY dir SUM(delta)`
- 支持任意时间范围（不依赖 scan 时间点）。

**独立模式（无 daemon）**：无 delta 功能。TUI 以纯 ncdu 模式运行，只显示 `name + size`。

### 1.4 Delta 作为覆盖层（Daemon 模式）

Delta 不是独立的"模式"，而是文件树上的一个**可视化覆盖层**（未来 daemon 模式）：
...
- **无 daemon**：纯 ncdu 模式，文件树只显示 `name + size`，无 delta 列
- **有 daemon 且有时间选择时**：树上的文件名旁显示 delta 值。树结构仍然来自当前文件系统，delta 只作为覆盖层叠加到节点上
...
```
TUI 启动 → 从 SQLite materialize 当前 cwd 的最新扫描结果 → 确定 cwd 作为树根
                │
                ├── scan_cache 有 cwd 的数据 → 以 FS 结构为底叠加 size
                │
                ├── scan_cache 无 cwd 的数据
                │       └── 配置 auto_scan_on_start = true
                │               → 后台扫描 cwd → 完成后刷新 size overlay
                │       └── 配置 auto_scan_on_start = false
                │               → list_dir(cwd) → 渲染 FS 树（目录 size="-"，文件有真实 size）
                │               → 用户按 s → scan_path(cwd) → 保存 + 更新缓存 → 刷新 size overlay
                │
                └── 用户导航到其他目录（l/h 切换树根）
...
2. **两层数据驱动**：
   ...
   - Scan 层（`scan_cache`）：为当前 FS 结构补充汇总 size；历史记录用于 delta 查询

3. **没有"路径选择器"**：按 `s` 不再弹输入框，直接扫描当前树根（cwd）。要切换扫描目标，先导航到目标目录再按 s。

4. **硬性规则 — 文件树永远反映当前文件系统状态**：文件树始终由 `list_dir`（即时文件系统列表）驱动，`scan_cache` 只补充 size，diff 数据只补充 delta。扫描记录（历史快照）和 diff 数据**只影响每个树节点上展示的 size / delta**，绝不改变树的结构（节点是否出现、能否展开/收起均不受 filter 影响）。时间选择器为空 = 无 delta 列。选择了时间范围 = 显示 delta。当 `from == to`（同一时间点）时，文件树展示当前文件系统视图，不显示 delta。

5. **统一数据模型**：数据库以 `(path, size, timestamp)` 为核心，并保留文件类型和必要元数据。TUI 可以查询任意子树在任意时间范围的 delta，无需"两个快照"的概念。

6. **独立模式 vs 服务模式**：两种模式都使用相同的 tree + delta filter 界面，只是 delta 数据的来源不同（SQLite 聚合 vs daemon 增量视图）。
