...
- 当前实现以 SQLite 为主存储，后续若需要导出或调试格式，可再评估其他输出方式。
...
| 实现 | 适用阶段 | 原理 |
|------|---------|------|
| **快照对比（Snapshot Diff）** | 旧方案 / 兼容思路 | 加载两份完整树并在内存中计算 diff。仅支持固定两个时间点对比，且必须属于同一树根路径 |
| **时序查询（Time-Series Query）** | 当前主方案 | SQLite 存储 `(path, size, timestamp)` 及必要元数据。支持任意时间范围聚合查询，如 `-2h`、`上周一到这周一` |

两种查询都输出相同的 `DiffNode` 结构。时序查询是对传统快照对比的升级版，将"一次性对比两个全量快照"替换为"按需从数据库聚合子树 delta"。

**独立模式**：TUI 启动时先根据最近一次扫描结果确定当前树根路径，再从 SQLite 中加载该树根路径的可用时间点做 diff。用户选择 from/to 两个时间点后触发后台 delta 查询。
...
- **有时间选择时**：树上的文件名旁显示 delta 值。树结构仍然来自当前文件系统，delta 只作为覆盖层叠加到节点上
