...
- **有 daemon 且有时间选择时**：树上的文件名旁显示 delta 值。树结构仍然来自当前文件系统，delta 只作为覆盖层叠加到节点上；如果某个目录已经有 `is_agg` 汇总记录，它代表整棵子树，不应再和同一子树的叶子明细重复相加
...
Delta 的可见数值必须遵循“子树覆盖”原则：

- 父目录汇总值表示整棵子树的净变化，不是当前屏幕上可见叶子行的简单求和。
- 如果查询结果里已经包含更高层的 `is_agg` 行，它覆盖的后代明细不应再参与同一次展示。
- 用户看到的目录 delta，应理解为“该目录子树的变化”，不是“展开状态下所有可见条目的 delta 总和”。

## 2. 守护进程 (argusd) - Phase 3

### 2.1 首次冷启动

- ~~对配置的用户目录进行全盘极速扫描，建立基准索引。~~(废弃)
...
### ~~2.4 时间轴快照 (Timeline) ~~ (废弃, 数据库只存储文件变化事件数据)
...
### 5.0 文件树与 size overlay

独立模式下，TUI 展示一个纯 ncdu 风格的文件树（仅 `name + size`）。delta 是未来 daemon 模式的可选覆盖层，不在独立模式范围内。

```
TUI 启动 → 确定 cwd 作为树根（scan_cache 为空）
                │
                ...
                │               → 用户按 s → scan_path(cwd) → 更新 scan_cache → 刷新 size overlay
                │
                └── 用户导航到其他目录（l/h 切换树根）
...
2. **两层数据驱动**：
   ...
   - Scan 层（`scan_cache`）：为当前 FS 结构补充汇总 size

3. **没有"路径选择器"**：按 `s` 不再弹输入框，直接扫描当前树根（cwd）。要切换扫描目标，先导航到目标目录再按 s。

4. **硬性规则 — 文件树永远反映当前文件系统状态**：文件树始终由 `list_dir`（即时文件系统列表）驱动，`scan_cache` 只补充 size。扫描记录**只影响每个树节点上展示的 size**，绝不改变树的结构（节点是否出现、能否展开/收起均不受影响）。

5. **无 delta**：独立模式下文件树只显示 `name + size`，无 delta 列。Delta 是 Phase 3+ daemon 模式的特性（参见 §1.3-1.4）。
