...
**无双快照时**（单次扫描或首次启动），不显示 delta 列，文件树退化为纯体积展示模式。详见 §5.0 TUI 启动三态。
...
TUI 客户端启动时根据 `~/.config/argus/snapshots/` 下的快照文件状态决定呈现模式。

**扫描路径是会话级选择，不是全局配置**：用户每次启动 TUI 可以选择探索不同目录（如今天扫 `~/Downloads`，明天扫 `~/Projects`）。快照通过 `root_path_hash` 按路径隔离。`~/.config/argus/snapshots/` 下可能包含多个路径的快照，TUI 在路径间切换时重新计算各自的状态。

**服务模式（连接 argusd）下不存在此三态**：守护进程管理快照生命周期，TUI 通过 UDS 直接获取 diff 数据，无需关心本地快照文件。详见 `02-architecture.md §3.1`。

| 状态 | 条件 | 行为 | 显示 |
|------|------|------|------|
| **Fresh** | 无任何快照文件 | 提示用户输入路径，触发首次扫描 | 单次文件树，**无 delta 列** |
| **Single** | 仅某路径有 1 个快照 | 自动加载（多路径时显示选择器让用户选），状态栏提示"仅一次扫描" | 文件树显示 `name + size`，**无 delta 列**；提供"再次扫描"入口 |
| **Dual** | 某路径有 ≥2 个快照且 `root_path_hash` 一致 | 加载最新两个快照，自动 compute diff（多路径时显示选择器） | 文件树显示 `name + size + delta` |
| **Dual** | 有 ≥2 个快照且 `root_path_hash` 一致 | 加载最新两个快照，自动 compute diff | 文件树显示 `name + size + delta` |

**核心原则**：delta 为差分数据，非必需基线。无两个快照时 TUI 退化为纯文件树浏览器（类似 ncdu），仅展示当前体积。

TUI 的三种模式是对"独立模式（Standalone）"的细化。服务模式（连接 argusd）下由守护进程管理快照生命周期，客户端不关心这三种状态（Phase 3+）。
