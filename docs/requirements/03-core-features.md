...
| 实现 | 适用阶段 | 原理 |
|------|---------|------|
| **快照对比（Snapshot Diff）** | Phase 1 CLI / Phase 2 TUI 独立模式 | 加载两个 JSON 快照，Tree Merge 算法在内存中计算 diff。仅支持"固定两个时间点"对比 |
...
**Phase 2 独立模式使用快照对比**：用户扫描一次生成一个快照，扫第二次后 TUI 自动取该路径下最近的两个快照做 diff。用户通过时间选择器指定要对比的时间点（选择器列举该路径下的可用快照时间戳）。
...
```
┌────────────────────────────────────────────┐
│  [/home/user]  时间: [2026-06-01 → 2026-07-01]  Δ≥ [50 MB]  [清除] │
├────────────────────────────────────────────┤
│  name              size        delta       │
...
TUI 启动 → 确定树根路径 → 渲染文件树
                ...
                │
                ├── 有该路径的快照 → 加载最新快照，渲染树（仅 size，无 delta）
                │                     顶部筛选栏显示可用时间范围
...
1. **没有"路径选择器"**：TUI 显示的树始终是某个扫描根的子树。首次使用时提示用户扫描一个目录。之后树根默认为上次扫描的路径，可通过重新扫描切换。
