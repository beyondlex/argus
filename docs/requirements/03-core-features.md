...
- **有时间选择时**：树上的文件名旁显示 delta 值。根树节点从 SQLite 获取 delta，子节点按相同时间范围计算
...
4. **硬性规则 — 文件树永远反映当前文件系统状态**：文件树始终由 `scan_cache`（最新的全量扫描结果）或 `list_dir`（即时文件系统列表）驱动。扫描记录（历史快照）和 diff 数据**只影响每个树节点上展示的 delta 值**，绝不改变树的结构（节点是否出现、能否展开/收起均不受 filter 影响）。时间选择器为空 = 无 delta 列。选择了时间范围 = 显示 delta。当 `from == to`（同一时间点）时，文件树展示当前文件系统视图，不显示 delta。
