...
4. **Delta 是筛选条件，不是模式**：时间选择器为空 = 无 delta 列。选择了时间范围 = 显示 delta。仅对当前树根路径的历史快照生效。当 `from == to`（同一时间点）时，文件树展示该时间点的完整快照视图，不显示 delta。文件树始终展示文件列表，不受筛选状态影响；筛选仅决定每个文件节点上是否展示 delta 变化量。
