...
|  +---------------------------------------------------------+  |
|  |                    argus-core                            |  |
|  |  - FileNode/Snapshot arena 数据结构                        |  |
|  |  - ScanCache 内存会话缓存（Arc 共享，COW 修改）            |  |
|  |  - 并行扫描器 (基于 jwalk)                                |  |
|  |  - disk_usage 跟踪（blocks × block_size）                 |  |
|  +---------------------------------------------------------+  |
+---------------------------------------------------------------+
```
...
**独立模式下的文件树**：TUI 使用扁平（ncdu-like）目录浏览模式。一次只显示当前目录的直接子项，
通过导航栈（dir_stack）管理目录进入/返回。所有行为通过调用 `argus-core` 实现，不依赖外部服务。
...
| **Scan 层** | `scan_cache` 缓存当前会话的扫描结果（Arc 共享） | 仅扫描后 |

- 文件始终展示 `disk_usage` 或 `size`（来自 FS `stat`）
- 目录有扫描汇总大小时展示汇总值，否则使用 `stat` 值
- 平面模式无结构占位节点概念
...
enum ArgusRequest {
    GetDelta { path: PathBuf, from: u64, to: u64, include_entries: bool },
...
| 组件 | 技术 | 选型理由 |
|------|------|---------|
| 核心扫描 | `jwalk` | 并行目录遍历，自动忽略 .gitignore，比 `ignore` 快 3-5x |
