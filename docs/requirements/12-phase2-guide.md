...
| 多路径快照 | 全量加载到 scan_cache，用于 size overlay |
...
3. **无快照或不匹配时不报错**：降级为 FS 导航模式，树展示目录结构（文件有真实 size，目录显示 `"-"`；结构占位节点显示 `"..."`）。用户按 `s` 扫描获得数据后自然升级。
...
- 列格式：`name` + `size`
- Phase 2 支持按名称 / 体积排序（`o` 键切换）；排序仅作用于当前展开层级，不改变整棵树的聚合结果。
...
```
[Tree] | ~/code/github  Size: 17.13 GB  Items: 104,690  Took: 25.28s ⠏ (press esc cancel)
```

底部固定行，不随文件树滚动。显示：
- 只显示 `[Tree]` 焦点标签
- 扫描中：实时 `Size`、`Items`、`Took`、spinner 和取消提示
- 扫描完成后：最近一次扫描摘要（路径、`Size`、`Items`、`Took`）
...
第 3 步：app.rs 重构 — scan_cache、view_root_path、rebuild_tree
第 4 步：FileTree 组件 — `"-"` / `"..."` 渲染 + has_scan_data + 惰性展开
第 5 步：handler.rs — s 直接扫描树根 + h 上导航 + l 展开/进入
第 6 步：event.rs — 移除 empty/scan 弹窗
第 7 步：Metadata 面板 — 扫描状态展示
第 9 步：Status Bar — 扫描态摘要 + 焦点标签收窄
第 10 步：后台任务 — 扫描完成更新 scan_cache + rebuild_tree
第 10 步：删除交互 — DeletePrompt 模式 + 二次确认（不变）
第 11 步：Help Popup — 帮助面板（P1，更新快捷键说明）
第 12 步：测试编写 — 见 §7
第 13 步：手动验收测试
```
...
- [x] App 重构: `view_root_path`, `scan_cache`
...
- [ ] 跑通手动验收：无快照启动 → 导航 cwd → 按 s 扫描 → 删除确认
...
| 层 | 测什么 | 方式 | 断言目标 |
|----|--------|------|---------|
| **状态逻辑** | AppState 状态迁移、展开/收起逻辑 | 纯函数测试，无渲染 | 状态字段值、列表长度/顺序 |
...
| **端到端** | 完整链路：启动无快照 → 导航 cwd → 扫描完成 → 渲染树 | `TestBackend` + 消息通道模拟 | 多帧 states + buffer 序列 |

### 7.2 状态逻辑测试（纯单元测试）

```rust
#[test]
fn test_scan_cache_populates_on_complete() {
    let mut app = App::new();
    let snapshot = make_mock_snapshot();
    app.handle_message(AppMessage::ScanComplete(snapshot.clone()));
    assert!(app.scan_cache.contains_key(&snapshot.root_path));
}
```
...
```rust
#[test]
fn test_file_tree_renders_basic() {
...
```rust
#[test]
fn test_scan_then_browse_workflow() {
    // 1. 模拟启动（无快照）
    let mut app = App::new();
...
- TUI 中手动触发扫描时，状态栏显示累积 `Size`、`Items`、耗时 `Took`，并提示 `(press esc cancel)`
- 状态栏左侧显示 `[Tree]` 焦点标签
- 扫描可被 `Esc` 取消
...
**状态逻辑测试**：
- `test_scan_cache_populates_on_complete`：扫描完成后 scan_cache 正确更新
...
| 仅有一个快照 | 加载到 scan_cache，目录显示真实汇总 size |
...
| 多个路径的快照 | 全量加载到 scan_cache；按当前树根路径匹配 |
