...
独立模式下，TUI 始终以当前工作目录 (cwd) 为根的文件树。文件树由**两层数据**驱动：

| 方面 | 旧模型（废弃） | 新模型 |
|------|--------------|--------|
| 启动流程 | 检查快照 → 空白/提示扫描 → 弹输入框 | 加载 scan_cache + 以 cwd 为根 → 有数据直接展示，无数据 `list_dir` 惰性读 FS |
| 目录 size | 始终有汇总 size（从快照来） | 有扫描数据则展示，无则显示 `"-"` |
| 文件 size | 始终有 | 始终有（`stat` 单次调用低成本） |
| 按 `s` | 弹出路径输入框 | 直接扫描当前树根（cwd），不再弹输入框 |
| 路径切换 | 通过路径选择器切换 | 用 `h` 可向上导航到父目录（改变树根），用 `l` 向下展开 |
| Delta 控制 | 自动根据快照数量决定 | 顶部筛选栏控制；无时间选择时无 delta 列 |
| 多路径快照 | 混乱堆积 | 全量加载到 scan_cache，仅当前树根的历史用于 diff |
...
└─────┬─────┘
      │
      ├── 加载所有 JSON 快照到 scan_cache (HashMap<PathBuf, Snapshot>)
      │
      ├── 确定 cwd = std::env::current_dir()
      │
      ├── scan_cache 有 cwd 的数据？
      │     ├── 是 → 渲染完整数据树（size + children + 最新扫描时间）
      ...
      │     ├── scan_path(cwd) → 保存 JSON → 更新 scan_cache
      │     └── 渲染完整数据树
      │
      └── 用户在筛选栏选择时间范围
            ├── 有 ≥2 个该路径的快照 → 后台 diff → 显示 delta
            └── 缺少快照 → 提示"需再次扫描此路径以对比"
```
...
3. **Delta 始终可选**：筛选栏的时间选择器列出当前树根路径下可用的快照时间戳。Phase 2 仅支持选择两个时间点（from/to 都选）做 diff。用户选择后触发后台 `compare_trees` 任务。

4. **无快照或不匹配时不报错**：降级为 FS 导航模式，树展示目录结构（文件有真实 size，目录显示 `"-"`）。用户按 `s` 扫描获得数据后自然升级。
...
**Phase 2 仅支持"固定快照列表"选择**：可用时间列表源于 `~/.config/argus/snapshots/` 中当前树根路径的快照文件名解析。用户从列出的时间戳中选择 from 和 to，TUI 加载对应的 JSON 快照并计算 diff。
...
- [ ] FileTree 渲染: `"- "` 用于无扫描数据的目录、`has_scan_data` 样式
- [ ] handler: `s` 不再弹输入框，直接 `start_scan(app)`
- [ ] handler: `h` 在根节点时 `navigate_up()` 改变树根
- [ ] handler: `l` 展开时检查 scan_cache，有数据用扫描树，无数据用 list_dir
...
- 加载 `~/.config/argus/snapshots/` 下的所有快照到 scan_cache
- 树根 = 当前工作目录 (cwd)
- 展示四栏布局：筛选栏 | 文件树（左70%）| 元数据（右30%）| 底部状态栏
- **有 cwd 快照**：显示完整数据树（所有节点有 size）
- **无 cwd 快照**：显示 cwd FS 文件树（文件有真实 size，目录显示 `"-"`）
...
由于文件树从"快照驱动"改为"FS 层 + Scan 层"双源驱动，测试需覆盖：

**状态逻辑测试**：
- `test_load_all_snapshots`：解析快照目录，正确填充 scan_cache
...
- `test_startup_with_snapshots`：有快照 → 显示完整数据树
...
| 场景 | 行为 |
|------|------|
| 无快照（首次启动） | 以 cwd 为根展示 FS 树（目录 size=`"-"`，文件有真实 size）。按 `s` 扫描后升级为完整数据树 |
...
| 导航到有快照的目录 | 从 scan_cache 读取完整数据树 |
