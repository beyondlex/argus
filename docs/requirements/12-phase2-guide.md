...
| 方面 | 旧模型（废弃） | 新模型 |
|------|--------------|--------|
| 启动流程 | 检查快照状态 → Fresh/Single/Dual 三态 → 路径选择器 | 直接展示最近一次扫描路径的树（或提示按 `s` 扫描）；无路径选择器 |
| 路径切换 | 通过路径选择器切换 | 按 `s` 重新扫描其他路径 |
| Delta 控制 | 自动根据快照数量决定是否有 delta | 顶部筛选栏控制；无时间选择时纯 ncdu |
| 多路径快照 | 混乱堆积 | 仅用于时间选择器提供可用的时间戳列表 |

> **为什么移除路径选择器**：路径选择器对用户来说"有点怪，不知要选哪个"。快照文件随着扫描增多越来越乱。新模型让用户始终专注于一个树状视图——从最近一次扫描的根路径开始，delta 是该树上的可选覆盖层。
...
└──┬───┘
   ...
   └── 无快照 ──▶ 提示"按 s 扫描目录" ──▶ 用户按 s
                                              │
                                              └── 打开扫描路径输入框（不切换 AppMode）
                                                   │
                                                   └── 输入自定义路径或确认当前 cwd
                                                        │
                                                        └── 扫描(tokio task) → 记录为当前树根 → 渲染文件树
```

**核心原则**：

1. **没有路径选择器**：首次使用提示扫描 → 扫描后树根即该路径。之后每次启动先扫描 `~/.config/argus/snapshots/`，按时间戳选择最新快照，并将其 `root_path` 作为当前树根。按 `s` 可重新扫描当前根或输入新路径，并将新扫描结果设为当前根。
...
- [ ] 搭好 `App`、`AppMode`、`AppState`、`AppMessage`、`AppCommand`、`AppError`
- [ ] 实现启动流程：读取 `~/.config/argus/config.toml`，再从 `~/.config/argus/snapshots/` 选当前树根
- [ ] 实现独立模式下的首次空状态提示：`按 s 扫描目录`
- [ ] 实现 `s` 扫描流程：打开路径输入框，执行扫描，落盘快照，刷新树
- [ ] 实现 `FileTree` 的展开 / 收起 / 光标移动 / 滚动
- [ ] 实现 `FilterBar` 的 from / to / threshold / clear
- [ ] 实现 snapshot diff 加载与 `compare_trees`
- [ ] 实现 delta 列显示、阈值过滤、排序切换
- [ ] 实现 `Metadata` 面板与 `Status Bar`
- [ ] 实现 `DeletePrompt` 与安全删除前置检查
- [ ] 完成 `Help Popup` 的占位或 P1 留白处理
- [ ] 补齐单元测试、渲染测试、事件测试、集成测试
- [ ] 跑通手动验收：无快照启动、两次扫描、筛选对比、清除筛选、删除确认
