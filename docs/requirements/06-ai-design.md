...
{
  "<target_path>": {
    "label_detail": "具体来源实体名（如：Docker Buildx 缓存、pip wheel 缓存）",
    "description": "用途说明",
...
注意：label 字段由程序基于内置规则和用户配置自动确定，无需 AI 输出。
AI 只需提供 label_detail 作为更具体的来源描述。
...
注意：label 字段由程序基于内置规则和用户配置自动确定，无需 AI 输出。
AI 只需提供来源实体描述（label_detail 的等效内容）。
```
...
请简明扼要地告诉用户：
1. 这是什么软件或系统进程产生的？有什么用？（label_detail 内容）
