...
**未扫描目录**：无扫描数据的目录在 size 列显示 `"-"`（灰色），文件始终显示真实大小。
有扫描数据后（按 `s` 扫描树根后），目录显示汇总大小。
**无元数据节点**：跳过目录中的深层子孙节点（如 `node_modules/pkg/src/`）显示 `"..."`（灰色），表示结构占位符、无大小信息。
...
独立模式下，TUI 启动时以当前工作目录 (cwd) 为树根。所有快照加载到内存缓存中：
- 当前树根有扫描数据 → 展示完整树（含 size）
