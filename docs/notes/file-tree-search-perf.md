...
### 1.3 `refresh_filtered_lines` 用过期 tree_idx 剪枝
...
更严重: `DeltaData` handler (`app.rs:307-316`) **只调** `refresh_filtered_lines()` 不调 `recompute_matches()`. 若 tree 结构在此期间变化, `tree_idx` 全错.

### 1.4 Delta 排序在 DeltaData 后不刷新
...
| 位置 | 问题 | 单次开销 |
|------|------|----------|
| `search.rs:12` 搜索每键 | `recompute_matches()` 全树遍历 | O(90K) + 排序 |
...
| 3 | P0 | `refresh_filtered_lines` 不剪枝 `match_indices`, 剪枝逻辑归 `recompute_matches` | `app.rs` |

### 3.2 性能修复 (影响体验)

| # | Fix | 方法 |
|---|-----|------|
| 4 | 搜索输入防抖 | Input 模式只存字符, Enter 才全树遍历 |
| 5 | `flatten_snapshot_tree` 减少分配 | path 复用 Vec, size_for_path 重用 PathBuf |
| 6 | `MAX_DIR_CHILDREN` 降至 500 | 防止单目录展开冻结 |
| 7 | `refresh_filtered_lines` 增量更新 | 只在 delta_cache 变更的行重新过滤, 而非全量 |

### 3.3 架构修复 (长远)

| # | Fix | 方法 |
|---|-----|------|
| 8 | SearchMatch.tree_idx 改为严格由 tree_lines index 驱动 | 重建时统一分配, 不用 visible_count 隐式计算 |
| 9 | 引入 dirty flag | tree_lines 未变时跳过 recompute_matches |
