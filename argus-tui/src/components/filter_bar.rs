...
pub fn render(
    ...
    let from_str = if let Some(idx) = filter.from_idx {
        snapshots.get(idx).map(|s| s.timestamp.format("%Y-%m-%d %H:%M").to_string()).unwrap_or_default()
    } else {
        String::new()
    };
    ...
    let threshold_str = filter.threshold.map(|t| util::format_size(t)).unwrap_or_default();

    let _can_diff = has_enough_snapshots && filter.from_idx.is_some() && filter.to_idx.is_some();
    let diff_hint = if !has_enough_snapshots {
        " (need 2 snapshots)"
    } else if filter.from_idx.is_some() && filter.to_idx.is_some() && filter.from_idx == filter.to_idx {
    ...
    let spans = vec![
        ...
        Span::raw(" → "),
        Span::styled(to_str, if focus { Style::default().fg(Color::Yellow).bg(Color::DarkGray) } else { Style::default().fg(Color::Cyan) }),
        Span::raw("]"),
        Span::raw("  Δ≥ ["),
        Span::styled(threshold_str, if focus { Style::default().fg(Color::Yellow).bg(Color::DarkGray) } else { Style::default().fg(Color::Magenta) }),
        Span::raw("]"),
        Span::raw("  "),
